...
def _kelly_multiplier(confidence="medium"):
    ...
    try:
        ...
        for r in sells:
            ...
            bp = avg_buys.get(r['ticker'], float(r['price']))
            if bp <= 0: continue
            pnl = (fill - bp) / bp
        ...
        pr = (sum(wins)/len(wins)) / (sum(losses)/len(losses))
        k = wr - (1-wr)/pr
        if k <= 0: return _KELLY_MIN_MULT
        m = 1.0 + k * _KELLY_FRACTION
        cs = {'high':1.0,'medium':0.7,'low':0.4}.get(confidence, 0.7)
        m = 1.0 + (m-1.0)*cs
        return max(_KELLY_MIN_MULT, min(_KELLY_MAX_MULT, m))
    except Exception:
        return 1.0
